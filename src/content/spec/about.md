...
at the same time. After spending multiple years between them I ultimately 
...
**horizontal knowledge** (ie. broad) both in computer & natural sciences and
...
2. Let Nix completely manage docker image builds via `pkgs.dockerTools`
...
I recently returned back to cubing after an unfortunate 10 years long pause. I was a
...
- **3x3**: all time PB 8.21s, my current local PB is 8.99s 
  (`R2 B R2 L' U' F2 U' L U2 D2 L2 D2 R2 B L2 D2 F D2 F' B'`), 11.47s ao5, 12.40s 
  ao12 and higher averages between 12-14s
...
- **4x4**: 48.48s, 54.06s ao5, 55.52s ao12
...
- **5x5**: 1:51.16s, 2:03.47s ao5, 2:05.44s ao12
